...
export default function Testimonials({ title, subtitle, testimonials, variant = "default" }: TestimonialsProps) {
  const getContainerClasses = () => {
    switch (variant) {
      ...
      default:
        return "mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"
    }
  }
      ...
      <div className="container mx-auto px-4 md:px-12 lg:px-20">
        ...
        <div className={getContainerClasses()}>
          {testimonials.map((testimonial, index) => (
            <div
              key={index}
              className="rounded-xl bg-white p-6 shadow-md hover:shadow-lg transition-shadow duration-300 flex flex-col h-full"
            >
              {testimonial.rating && (
