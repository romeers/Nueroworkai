...
export default function TestContactDbPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log("Enviando datos a la base de datos:", { name, email, message })

      const response = await fetch("/api/test-contact-db", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="container mx-auto py-10">
      <h1 className="text-2xl font-bold mb-6">Prueba de Contacto con Base de Datos</h1>

      <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6">
        <p className="text-yellow-700">
          Esta es una página de diagnóstico para probar el guardado de mensajes en la base de datos. No es parte del
          sitio público.
        </p>
      </div>
        ...
        <div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" disabled={loading}>
              {loading ? "Guardando..." : "Guardar en la base de datos"}
            </Button>
          </form>
        </div>
